...
def test_create_user_should_hash_password_before_saving(service):
    service.repository.get_by_email.return_value = None
    service.repository.get_by_login.return_value = None
    ...
    created_object = service.repository.create.call_args.args[0]
    assert created_object.password != "123456"
    assert verify_password("123456", created_object.password)


def test_can_register_without_auth_when_no_user_exists(service):
    service.repository.has_any_user.return_value = False

    assert service.can_register_without_auth()


def test_cannot_register_without_auth_when_user_exists(service):
    service.repository.has_any_user.return_value = True

    assert not service.can_register_without_auth()
